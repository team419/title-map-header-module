  ...
  return (
    <div className={`${css['css.title']} ${css['title-left']}`}>
      <div className={css['title-left-entry']}>
        ...
        <span className={css['title-business-claimed']}>Claimed</span>
      </div>

      <div className={`${css['title-left-entry']} ${css['left-entry-1']}`}>
        <img alt="rating" className={css['img-rating']} src={ratingFourStar} />
        <span className={css['review-count']}>{`${business.totalReviews} reviews`}</span>
        <button type="button" className={css['btn-title-gray']} onClick={toggleModalDetails}>
          <svg role="img" className={`${css['icon-svg']} ${css['svg-histogram']}`} viewBox="0 0 14 14">
      ...
      <div className={css['title-left-entry']}>
        <span className={css['average-price']}>{setPrice(business.averageCost)}</span>
